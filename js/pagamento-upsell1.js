import { getOrGeneratePix, preloadUpsell2Pix, clearOldCache } from './pix-preloader.js';
...
async function createPixPayment() {
  try {
    ...
    const pixData = await getOrGeneratePix('upsell1', 2290);
...
async function verificarPagamento(idTransacao) {
  try {
    ...
    console.error('[Pagamento Upsell1] Erro ao verificar pagamento:', error);
    return false;
  }
}
...
function iniciarVerificacaoPagamento(idTransacao) {
  ...
  console.log(`[Pagamento Upsell1] Iniciando verificação de pagamento para transação: ${idTransacao}`);
  ...
  checkPaymentInterval = setInterval(async () => {
    ...
    if (attempts >= maxAttempts) {
      console.log(`[Pagamento Upsell1] Máximo de tentativas (${maxAttempts}) atingido`);
      clearInterval(checkPaymentInterval);
      return;
    }

    console.log(`[Pagamento Upsell1] Tentativa ${attempts}/${maxAttempts}`);
    ...
    if (isPaid) {
      ...
      showTransition('upsell1ToUpsell2', () => {
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    console.log('[Pagamento Upsell1] Página em background - polling continua');
  } else {
    console.log('[Pagamento Upsell1] Página voltou ao foco');
  }
});
